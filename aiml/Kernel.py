...
class Kernel:
    ...
    def learn(self, filename):
        ...
        for f in glob.glob(filename):
            ...
            if self._verboseMode:
                # print( "done (%.2f seconds)" % (time.perf_counter()-start) )   # (time.clock() - start) )
    ...
    def respond(self, input_, sessionID = _globalSessionID):
        ...
        try:
            ...
            for s in sentences:
                ...
                # response, so that <input/> tags work properly. ????? <input/> ???????
    ...
    def _respond(self, input_, sessionID):
        """Private version of respond(), does the real work. respond() ????????????"""
        if len(input_) == 0:
            return u""
